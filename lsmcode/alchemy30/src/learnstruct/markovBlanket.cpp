...
MarkovBlanketsFinder::MarkovBlanketsFinder(FeatureArray* featArray,
                                           ...
                                           Array<Domain*>*const& domains) :
	featArray_(featArray), threshold_(threshold),
  countIsAtLeast_(countIsAtLeast), domains_(domains)
{
...
void MarkovBlanketsFinder::constructClausesFromMB(
    ClauseOpHashArray& constructedClauses, const int& maxPreds /*TODO:*/)
{
	makeIntArrayClauses();
	makeActualClauses(constructedClauses, maxPreds);
}
...
{
	growShrink();
	constructClausesFromMB(constructedClauses, maxPreds);
}
         ...
         ClauseOpHashArray & constructedClauses, const int& maxPreds /*TODO:*/)
{
    ...
    cout << "maxPreds = " << maxPreds << endl; //TODO: REMOVE
		if (newClause->getNumPredicates() <= maxPreds &&
        constructedClauses.append(newClause) >= 0) {
			Array<Clause *> flipClauses;
			flipClauses.append(newClause);

      //TODO: ADDED 
      cout << "BUSL_ORIG_CAND " << constructedClauses.size()-1 << ": ";
			newClause->print(cout, (*domains_)[0]);
      cout << endl;
          ...
          //TODO: CHANGED to fix bug of not creating all possible clauses
